...
Pronto! Você pode adicionar outros eventos na classe Events usando esse mesmo formato, com o decorator `@Cog.listener()` e em baixo a função do evento (sempre com o `self` como primeiro parâmetro). Lembrando que os eventos recebem parâmetros diferentes, você pode verificar a lista com todos eles [clicando aqui](https://docs.discord4py.dev/en/developer/api/events.html).